...
class RequestOTPView(APIView):
    ...
    def post(self, request):
        ...
        if not wa_success:
            # WhatsApp service unavailable — expose OTP in DEV mode so testing can continue
            print(f"[OTP DEV FALLBACK] WhatsApp send failed. Code for {phone_number}: {code}")
            return Response({
                'detail': 'WhatsApp service unreachable. OTP generated — see dev_otp field.',
                'phone_number': phone_number,
                'dev_otp': code,
            }, status=status.HTTP_400_BAD_REQUEST)
...
class ExpiryRiskPredictionView(APIView):
    ...
    def get(self, request):
        ...
        near_expiry_products = Product.objects.filter(expiry_date__lte=in_14_days).select_related('category')

        if not near_expiry_products.exists():
            near_expiry_products = Product.objects.all().select_related('category')
